...
function App() {
  ...
  const fetchWeather = useCallback(
    async (searchCity) => {
      ...
      try {
        const geoResponse = await axios.get(GEOCODING_API_URL, {
          params: { name: searchCity, count: 1, language: 'pt', format: 'json' },
        });
    ...
    [GEOCODING_API_URL, WEATHER_API_URL]
  );
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        try {
          ...
          const cityName =
            geoResponse.data.city || geoResponse.data.locality || 'Localização atual';
  ...
  return (
    <div
      ...
      style={{
        backgroundImage: `linear-gradient(to bottom, ${isDark ? 'rgba(15, 23, 42, 0.4), rgba(15, 23, 42, 0.5)' : 'rgba(255, 255, 255, 0.4), rgba(219, 234, 254, 0.5)'}), url(${weatherBg})`,
      }}
    >
      <button
